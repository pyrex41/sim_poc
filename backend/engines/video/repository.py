"""
Video generation database repository.
"""
import json
import logging
...
from .models import VideoResponse, VideoListItem
...
class VideoRepository:
    ...
    async def create_task(self, task: Task) -> Task:
        ...
        logger.info(f"Created video task {task.id} in database")
        return task
    ...
    async def get_task(self, task_id: str) -> Optional[Task]:
        ...
        row = self.db.execute_query(
            query,
            (task_id, EngineType.VIDEO.value),
            fetch_one=True
        )
    ...
    async def update_task(
        ...
        params.extend([task_id, EngineType.VIDEO.value])
        ...
        logger.info(f"Updated video task {task_id}")
    ...
    async def save_video_result(
        ...
        """
        Save the final video result.

        Args:
            task_id: Task ID
            video_data: Video result data
