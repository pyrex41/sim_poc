...
class ReplicateVideoProvider(BaseVideoProvider):
    """Replicate implementation for video generation."""
    ...
    def __init__(self, api_key: str = None):
        ...
        logger.info("ReplicateVideoProvider initialized")
    ...
    async def submit_task(self, params: Dict[str, Any]) -> str:
        ...
        """
        try:
            logger.info(f"Submitting video task to Replicate: {params.get('model', 'unknown')}")

            model_identifier = self._get_model_identifier(params.get('model', 'minimax'))
            ...
            response = self.session.post(
                ...
                timeout=60  # Videos can take longer to start
            )
            ...
            logger.info(f"Replicate video task submitted: {prediction_id}")
    ...
    async def get_task_status(self, provider_task_id: str) -> Dict[str, Any]:
        ...
        try:
            ...
            if status == 'succeeded':
                ...
                # Replicate video output can be a URL or array of URLs
                video_url = output[0] if isinstance(output, list) else output

                result_data['result'] = {
                    'url': video_url,
                    'format': 'mp4',  # Default, could be extracted from URL
                }
    ...
    def transform_request(self, request: BaseRequest) -> Dict[str, Any]:
        """
        Transform our VideoRequest to Replicate format.

        Args:
            request: Our VideoRequest object

        Returns:
            Replicate-formatted parameters
        """
        if not isinstance(request, VideoRequest):
            raise ValueError("Request must be a VideoRequest")
        ...
        if model_config:
            ...
            if "first_frame_image" in optional_inputs and request.first_frame_image:
                replicate_input["first_frame_image"] = request.first_frame_image

            if "image" in optional_inputs and request.image:
            ...
            if "loop" in optional_inputs and request.loop is not None:
                replicate_input["loop"] = request.loop

            if "prompt_optimizer" in optional_inputs and request.prompt_optimizer is not None:
                replicate_input["prompt_optimizer"] = request.prompt_optimizer

            if "images" in optional_inputs and request.images:
                replicate_input["images"] = request.images

            if "fps" in optional_inputs and request.fps:
                replicate_input["fps"] = request.fps
    ...
    def _get_model_identifier(self, model: str) -> str:
        ...
        if model_config and model_config.category == ModelCategory.VIDEO:
