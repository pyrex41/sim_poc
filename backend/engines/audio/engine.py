...
from .providers import ReplicateAudioProvider


logger = logging.getLogger(__name__)


class AudioEngine(BaseGenerationEngine[AudioRequest, AudioResponse]):
    """Audio generation engine with provider abstraction."""

    def __init__(self, repository: Optional[AudioRepository] = None):
        """
        ...
        self.providers = {
            'replicate': ReplicateAudioProvider(),
        }

        logger.info("AudioEngine initialized")

    async def generate(self, request: AudioRequest, user_id: str) -> Task:
        """
        ...
        try:
            ...
            logger.info(f"Audio task {task_id} submitted to {provider_name}: {provider_task_id}")

        except Exception as e:
            # Update task with error
            logger.error(f"Failed to submit audio task {task_id}: {e}")
            task = await self.repository.update_task(
                task_id,
                {
    ...
    async def get_result(self, task_id: str, user_id: str) -> AudioResponse:
        """
        Get the final audio result.
        ...
        return await self.repository.save_audio_result(task_id, task.result)
    ...
    async def cancel(self, task_id: str, user_id: str) -> bool:
        ...
        logger.info(f"Canceled audio task {task_id}")
        return True

    async def list_tasks(self, filters: TaskFilter) -> PaginatedResponse[Task]:
        """
        List audio generation tasks.
