"""
Image generation database repository.
"""
import json
import logging
...
from .models import ImageResponse, ImageListItem
...
class ImageRepository:
    ...
    async def create_task(self, task: Task) -> Task:
        ...
        logger.info(f"Created image task {task.id} in database")
        return task
    ...
    async def get_task(self, task_id: str) -> Optional[Task]:
        ...
        row = self.db.execute_query(
            query,
            (task_id, EngineType.IMAGE.value),
            fetch_one=True
        )
    ...
    async def update_task(
        ...
        params.extend([task_id, EngineType.IMAGE.value])
        ...
        logger.info(f"Updated image task {task_id}")
    ...
    async def save_image_result(
        ...
        """
        Save the final image result to the generated_images table.
        This maintains backward compatibility with existing schema.

        Args:
            task_id: Task ID
            image_data: Image result data
        ...
        query = """
            INSERT INTO generated_images (
                prompt, image_url, model_id, parameters, status,
                collection, metadata, client_id, campaign_id, created_at
            ) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
        """
        ...
        image_id = self.db.execute_query(query, params)
