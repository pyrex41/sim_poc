...
from .models import UtilityResponse, UtilityListItem
...
class UtilitiesRepository:
    """Repository for utilities database operations."""
    ...
    async def create_task(self, task: Task) -> Task:
        """
        Create a new utility processing task.
        ...
        logger.info(f"Created utility task {task.id} in database")
        return task
    ...
    async def get_task(self, task_id: str) -> Optional[Task]:
        ...
        row = self.db.execute_query(
            query,
            (task_id, EngineType.UTILITIES.value),
            fetch_one=True
        )
    ...
    async def update_task(
        ...
        params.extend([task_id, EngineType.UTILITIES.value])
        ...
        logger.info(f"Updated utility task {task_id}")
    ...
    async def list_tasks(
        ...
        params = [EngineType.UTILITIES.value]
    ...
    async def save_utility_result(
        self,
        task_id: str,
        utility_data: Dict[str, Any]
    ) -> UtilityResponse:
        """
        Save the final utility result.

        Args:
            task_id: Task ID
            utility_data: Utility result data
        ...
        return UtilityResponse(
            ...
            tool=task.params.get('tool'),
            provider=task.provider,
            input_url=task.params.get('image') or task.params.get('video'),
            output_url=utility_data.get('url'),
            width=utility_data.get('width'),
            height=utility_data.get('height'),
            file_size=utility_data.get('file_size'),
            format=utility_data.get('format'),
            parameters=task.params,
            user_id=task.user_id,
            client_id=task.client_id,
            campaign_id=task.campaign_id,
            local_path=utility_data.get('local_path'),
        )
