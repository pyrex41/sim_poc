module VideoToText exposing (Model, Msg(..), init, update, view)
...
type alias Model =
    { models : List VideoToTextModel
    , selectedModel : Maybe VideoToTextModel
...
init =
    ( { models = []
      ...
      , error = Nothing
      , searchQuery = ""
      , selectedCollection = "video-to-text"
      , requiredFields = []
...
type Msg
    ...
    | ModelsFetched (Result Http.Error (List VideoToTextModel))
    | SelectModel String
    ...
    | UpdateSearch String
    | GenerateText
    | TextGenerated (Result Http.Error { video_id : Int, status : String })
    | NavigateToResult Int
    | PollStatus
    | StatusFetched (Result Http.Error VideoToTextRecord)
    | ScrollToModel (Result Dom.Error ())
    | FileSelected String File
    | VideoUploaded String (Result Http.Error String)
    ...
    case msg of
        ...
        SelectCollection collection ->
            ...
            if model.selectedCollection == collection then
                ...
                ( { model | selectedCollection = collection, selectedModel = Nothing, outputText = Nothing, requiredFields = [], selectedVersion = Nothing, pendingParameters = [], pendingModelSelection = Nothing }
        ...
        SelectModel modelId ->
            ...
            if List.isEmpty model.models then
                ...
                case selected of
                    Just selectedModel ->
                        ( { model | selectedModel = selected, outputText = Nothing, error = Nothing, parameters = [] }
                        , fetchModelSchema selectedModel.id
                        )
        ...
        GenerateText ->
            case model.selectedModel of
                Just selectedModel ->
                    ...
                    , generateText selectedModel.id model.parameters model.selectedCollection model.selectedVersion
                    )
        ...
        TextGenerated result ->
            case result of
                Ok response ->
                    ...
                    , Task.perform (\_ -> NavigateToResult response.video_id) (Task.succeed ())
                    )
        ...
        FileSelected paramKey file ->
            ...
            , uploadVideo paramKey file
            )

        VideoUploaded paramKey result ->
            case result of
                Ok videoUrl ->
                    let
                        updatedParams =
                            updateParameterInList paramKey videoUrl model.parameters
                    in
                    ( { model
                        | uploadingFile = Nothing
...
getDefaultParameters : VideoToTextModel -> List Parameter
...
view model =
    div [ class "video-page" ]
        [ h1 [] [ text "Video to Text Models Explorer" ]
        , div [ class "search-section" ]
            [ input
                [ type_ "text"
                , placeholder "Search video-to-text models..."
        ...
        , case model.selectedModel of
            Just selected ->
                div [ class "selected-model", id "selected-model-section" ]
                    ...
                    , button
                        ...
                        [ text (if model.isGenerating then "Generating..." else "Generate Text") ]
                    ]
            ...
            Nothing ->
                if not (List.isEmpty model.models) then
                    ...
                    text ""
        , case model.outputText of
            Just textOutput ->
                div [ class "text-output" ]
                    [ h3 [] [ text "Generated Text" ]
                    , pre [ class "output-text-content" ] [ text textOutput ]
                    ]
...
viewModelOption : VideoToTextModel -> Html Msg
...
viewParameter model param =
    let
        ...
        isVideoField =
            param.format == Just "uri" || String.contains "video" (String.toLower param.key)
...
fetchModels collection =
    Http.get
        ...
        , expect = Http.expectJson ModelsFetched (Decode.field "models" (Decode.list videoToTextModelDecoder))
        }
...
generateText : String -> List Parameter -> String -> Maybe String -> Cmd Msg
generateText modelId parameters collection maybeVersion =
    let
    ...
    Http.post
        ...
        , expect = Http.expectJson TextGenerated textResponseDecoder
        }

textResponseDecoder : Decode.Decoder { video_id : Int, status : String }
textResponseDecoder =
...
uploadVideo : String -> File -> Cmd Msg
uploadVideo paramKey file =
    Http.post
        { url = "/api/upload-video"
        , body = Http.multipartBody [ Http.filePart "file" file ]
        , expect = Http.expectJson (VideoUploaded paramKey) uploadResponseDecoder
        }
...
videoToTextModelDecoder : Decode.Decoder VideoToTextModel
videoToTextModelDecoder =
    Decode.map4 VideoToTextModel
...
demoModels : List VideoToTextModel
demoModels =
    [ VideoToTextModel "demo/video-to-text" "Demo Video-to-Text" "Generates text from video content" Nothing
    ]
